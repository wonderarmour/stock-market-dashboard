...
- 뷰: **개별 차트** / **한 그래프에 비교**(기간 시작 0% 정규화, 범례 클릭으로 숨김) / **비중 반영 합산**(슬라이더로 비중, 수익률·MDD·변동성, 조합 저장·비교·이름 인라인 수정, 롤링 창 분석).
...
## 7. 브라우저에 저장되는 것 (localStorage)

포트폴리오 설정은 계정 없이 브라우저별로 저장돼요. 다른 컴퓨터/브라우저에서는 비어 있는 게 정상이에요.

| 키 | 내용 |
|---|---|
| `portfolio-symbols` | 종목 목록 |
| `portfolio-weights` | 비중 입력값 |
| `portfolio-combos` | 저장한 비중 조합 |
| `portfolio-hidden` | 비교 차트에서 숨긴 종목 |
| `portfolio-rolling-window`, `portfolio-rolling-scope` | 롤링 분석 창·범위 |
| `tg-channels` | 텔레그램 채널 입력값 |

옮기려면 개발자도구(F12) → Application → Local Storage 에서 값을 복사해 새 브라우저에 붙여 넣어요.
